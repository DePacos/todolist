...
export const sliceTodolist = createTodolistSlice({
  ...
  reducers: (creators) => {
    return {
      ...
      createTodolist: creators.asyncThunk(async (title: string, { rejectWithValue }) => {
        try {
      ...
      updateTodolistTitle: creators.asyncThunk(async (arg: { todoId: string, title: string }, { rejectWithValue }) => {
        try {
        ...
        async (undefined, { rejectWithValue }) => {
          ...
          fulfilled: (state, action) => {
            return action.payload.todolist.map((todo) => ({
              ...todo,
              tasksFilter: action.payload.filter === "test" ? "all" : action.payload.filter,
              entityStatus: "idle"
            }))
          }
        }),

      changeTodolistStatus: creators.reducer((state, action: PayloadAction<{
        todoId: string,
      ...
      changeTodolistFilter: creators.reducer((state, action: PayloadAction<{ todoId: string, filter: Filter }>) => {
        const index = state.findIndex((todo) => todo.id === action.payload.todoId)
