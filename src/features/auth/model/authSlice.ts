...
import { asyncThunkCreator, buildCreateSlice, isFulfilled } from "@reduxjs/toolkit"
...
export const sliceAuth = createAuthSlice({
  ...
  reducers: (creators) => {
    return {
      setIsLoggedIn: creators.asyncThunk(async (data: LoginParams, { rejectWithValue }): Promise<any> => {
          try {
      ...
      initializeApp: creators.asyncThunk(async (_, { dispatch, rejectWithValue }) => {
          try {
      ...
      logout: creators.asyncThunk(async (_, { rejectWithValue }) => {
          try {
            const res = await authAPI.logout()
            if (res.data.resultCode === ResultCode.Success) {
              // TODO make deleting tasks
  ...
  extraReducers: builder => {
    builder.addMatcher(
      isFulfilled(authActions.setIsLoggedIn, authActions.initializeApp, authActions.logout),
      (state, action) => {
